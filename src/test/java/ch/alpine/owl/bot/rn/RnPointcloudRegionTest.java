...
public class RnPointcloudRegionTest extends TestCase {
  public void testEmpty() {
    ...
    assertFalse(region.isMember(Tensors.vector(2, 2.5)));
    assertFalse(region.isMember(Tensors.vector(2, 2)));
    assertFalse(region.isMember(Tensors.vector(2, 1)));
    assertFalse(region.isMember(Tensors.vector(7, 1)));
  }

  public void testSingle2D() {
    Region<Tensor> region = RnPointcloudRegion.of(Tensors.matrix(new Number[][] { { 2, 3 } }), RealScalar.ONE);
    assertTrue(region.isMember(Tensors.vector(2, 2.5)));
    assertTrue(region.isMember(Tensors.vector(2, 2)));
    assertFalse(region.isMember(Tensors.vector(2, 1)));
    assertFalse(region.isMember(Tensors.vector(7, 1)));
  }
  ...
  public void testTwo2D() {
    ...
    assertTrue(region.isMember(Tensors.vector(2, 2)));
    assertFalse(region.isMember(Tensors.vector(2, 1)));
    assertTrue(region.isMember(Tensors.vector(7, 1)));
    assertTrue(region.isMember(Tensors.vector(7, 2)));
    assertFalse(region.isMember(Tensors.vector(8, 2)));
  }
